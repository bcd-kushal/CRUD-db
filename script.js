...
dropTableButton.addEventListener( "click", () => {
    ...
    if( sureToDoThis( "This will delete the Table... Be Sure of this action." ) ){
        ...
        const php_connect_api_endpoint_url = "https://candies-production.up.railway.app/";
        const http_request_type = "DELETE";

        fetch( php_connect_api_endpoint_url, {
            method: http_request_type
        })
        ...
        /* 
        axios({
            url: php_connect_api_endpoint_url,
            method: http_request_type
        })
        .then((res) => {
            console.log(res);
...
//=========== download json table data ==============================================

const downloadJSONTableData = document.querySelector(".jsonDownloadButton");
downloadJSONTableData.addEventListener( "click", () => {
    
    if( sureToDoThis( "Table Data will be downloaded as JSON... Proceed?" ) ){
        alert("DOWNLOADING");
        ...
        try {
            ...
            alert("JSON data is malformed.");
            alert("COULDNT DOWNLOAD IN JSON")
            return;
        }
        ...
        var currentDateTime = String(currentDate.toLocaleString());
        console.log(currentDateTime);

        downloadJSON( ace_editor.getValue() || {}, "database"+currentDateTime, "text/json" );
...
//================ drop table onclick button ========================================

//const dropTableButton = document.getElementsByClassName("dropTableButton")[0];
